...
class AcquireTransformer : public rclcpp::Node {
 public:
  AcquireTransformer() : Node("transformer") {
    ...
    this->declare_parameter("output_topic", "stream0_transformed",
                            output_topic_desc);
 ...
 private:
  ...
  void transform_image(sensor_msgs::msg::Image::SharedPtr msg) {
    ...
    CHECK(sensor_msgs::fillImage(out_msg, "mono8", msg->height, msg->width,
                                 msg->width, img.data));
  ...
  void configure_subscriber_() {
    ...
    subscription_ = this->create_subscription<sensor_msgs::msg::Image>(
        topic, *qos,
        std::bind(&AcquireTransformer::transform_image, this,
                  std::placeholders::_1));
  }
